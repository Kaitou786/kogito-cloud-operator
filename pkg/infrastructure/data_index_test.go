...
func Test_getKogitoDataIndexURLs(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if (len(gotDataIndexEndpoints.HTTPRouteEnv) > 0 || len(gotDataIndexEndpoints.WSRouteEnv) > 0 || len(gotDataIndexEndpoints.WSRouteURI) > 0) && gotDataIndexEndpoints.HTTPRouteURI != tt.wantHTTPURL {
				t.Errorf("GetDataIndexEndpoints() gotHTTPURL = %v, want %v", gotDataIndexEndpoints.HTTPRouteURI, tt.wantHTTPURL)
			}
			if (len(gotDataIndexEndpoints.HTTPRouteEnv) > 0 || len(gotDataIndexEndpoints.WSRouteEnv) > 0 || len(gotDataIndexEndpoints.HTTPRouteURI) > 0) && gotDataIndexEndpoints.WSRouteURI != tt.wantWSURL {
